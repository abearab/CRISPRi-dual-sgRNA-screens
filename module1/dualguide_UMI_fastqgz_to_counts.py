            ...
            with gzip.open(fileTup[2]) as infile_r3:
                for i, (r1, r2, r3) in enumerate(zip(infile_r1, infile_r2, infile_r3)):
                    if i % 4 == 1:
                        ...
                        protospacer_a_r1 = matchBarcode(mismatchDicts['protospacer_a_r1'],
                                                        r1[:19].strip().decode("utf-8"), allowOneMismatch=False)
                        protospacer_b_r2 = matchBarcode(mismatchDicts['protospacer_b_r2'],
                                                        r3[:19].strip().decode("utf-8"), allowOneMismatch=False)
...
if __name__ == '__main__':
    ...
    parser.add_argument('Seq_File_Names', nargs='+',
                        help='Name(s) of sequencing file(s). Unix wildcards can be used to select multiple files at '
                             'once. The script will search for all *.fastq.gz, *.fastq, and *.fa(/fasta/fna) files '
